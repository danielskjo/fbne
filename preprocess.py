...
def innovation_get_random_walks_restart(datafile, uv):
    G = graph.load_edgelist(datafile, undirected=True)
    print("Folded HIN ==> number of nodes: {}".format(len(G.nodes())))
    print("walking...")
    walks = graph.innovation_build_deepwalk_corpus(G, None, 5, uv)
    print("walking...ok")
...
def innovation(path, history_u_lists, history_v_lists, edge_list_uv, edge_list_vu):
    ...
    BiG.add_weighted_edges_from(edge_list_uv + edge_list_vu)
    A = bi.biadjacency_matrix(
        BiG, node_u, node_v, dtype=np.uint32, weight='weight', format='csr')
    AT = A.transpose()

    row_index = dict(zip(node_u, itertools.count()))
    col_index = dict(zip(node_v, itertools.count()))

    first_order_user_A = A @ AT
    second_order_user_A = (first_order_user_A @ first_order_user_A).toarray()
    del first_order_user_A

    entries = []
    for i, row in enumerate(second_order_user_A):
        row[i] = 0
        prob = normalize(row.reshape(1, -1), norm='l1')

        for j in range(len(prob[0])):
            entries.append((i, j, float(prob[0][j])))

    sql = 'INSERT INTO `user_prob` (`row`, `col`, `prob`) VALUES (%s, %s, %s)'
    cursor.executemany(sql, entries)
    db.commit()
    print(cursor.rowcount, "was inserted into user_prob")

    index_row = dict(zip(row_index.values(), row_index.keys()))
    fw_u = os.path.join(path, "homogeneous_u.dat")
    save_homogenous_graph_to_file(sparse.csr_array(second_order_user_A), fw_u, index_row, index_row)
    G_u, walks_u = innovation_get_random_walks_restart(fw_u, True)

    del second_order_user_A

    first_order_item_A = AT.dot(A)
    del A
    del AT
    second_order_item_A = (first_order_item_A @ first_order_item_A).toarray()
    del first_order_item_A

    entries = []
    for i, row in enumerate(second_order_item_A):
        row[i] = 0
        prob = normalize(row.reshape(1, -1), norm='l1')
    ...
    sql = 'INSERT INTO `item_prob` (`row`, `col`, `prob`) VALUES (%s, %s, %s)'
    cursor.executemany(sql, entries)
    db.commit()
    print(cursor.rowcount, "was inserted into item_prob")

    index_item = dict(zip(col_index.values(), col_index.keys()))
    fw_v = os.path.join(path, "homogeneous_v.dat")
    save_homogenous_graph_to_file(sparse.csr_array(second_order_item_A), fw_v, index_item, index_item)
    G_v, walks_v = innovation_get_random_walks_restart(fw_v, False)

    del second_order_item_A
...
def preprocess(path):
    ...
    G_u, walks_u, G_v, walks_v = innovation(
        path, history_u_lists, history_v_lists, edge_list_uv, edge_list_vu)
