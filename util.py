    ...
    def calc_centrality(self, G):
        ...
        if os.path.exists(G.name+'/degree.txt') and\
            ...
            if not os.path.exists(G.name+'/betweenness.txt'):
                ...
                start_time = time.time()
                node_centrality_values["betweenness"] = nx.betweenness_centrality(
                    G)
            ...
            if not os.path.exists(G.name+'/eigenvector.txt'):
                ...
                start_time = time.time()
                node_centrality_values["eigenvector"] = nx.eigenvector_centrality_numpy(
                    G)
            ...
            if not os.path.exists(G.name+'/closeness.txt'):
                ...
                start_time = time.time()
                node_centrality_values["closeness"] = nx.closeness_centrality(
                    G)
    ...
    def adj_list_sorted_centrality(self, node_centrality_values, adj_lists):
        ...
        for centrality in node_centrality_values:
            for node in adj_lists:
                sorted_adj_lists[node] = set(
                    sorted(adj_lists[node], key=lambda x: node_centrality_values[centrality][x]))
                ...
                for count, node in enumerate(G.nodes()):
                    ...
                    for i in range(num_walks):  # for each node, generating #num of walks
                        ...
                        for _ in range(WALK_LEN):
                            ...
                            adj_prob_cur_node = [
                                float(i) for i in adj_prob[sampler][curr_node]]
                            adj_prob_cur_node_norm = [
                                i/sum(adj_prob_cur_node) for i in adj_prob_cur_node]
                            next_node = np.random.choice(
                                neighbors, p=adj_prob_cur_node_norm)
                            if curr_node != node:
                                pairs.append((node, curr_node))
                            curr_node = next_node
                print("Done", sampler, " walks for",
                      count, "nodes, writing to file ...")
    ...
    def negtive_sampling(self, G, net_type, uSet, vSet, k, node_centrality_values, file_path):
        ...
        if not os.path.exists(out_file) or not os.path.exists(out_negative1k):
            ...
            with open(out_file, "w") as fp:
                ...
                for node in G.nodes():
                    if net_type == 'u2u':
                        candi_set = set(nx.non_neighbors(
                            G, node)).intersection(uSet)
                        nonNeighbors = list(set([x for x in candi_set]))

                    if net_type == 'u2b':
                        if node in uSet:
                            candi_set = set(nx.non_neighbors(
                                G, node)).intersection(vSet)
                        else:
                            candi_set = set(nx.non_neighbors(
                                G, node)).intersection(uSet)
                        nonNeighbors = list(set([x for x in candi_set]))
                    ...
                    fp.write("\n")
                print("negative sampling for "+G.name +
            ...
            if not os.path.exists(out_negative1k) and net_type == 'u2b':
                with open(out_negative1k, "w") as wp:
                    ...
                    nodes_prob = defaultdict(list)
                    for node in uSet:
                        nonNeighbors = list(set([x for x in nx.non_neighbors(
                            G, node)]).intersection(vSet))  # hit ratio test
                        nodes_prob[node] = np.random.choice(
                            nonNeighbors, size=1000)
                        wp.write(str(node)+",")
                        for sample in nodes_prob[node]:
                            wp.write(str(sample)+",")
                        wp.write("\n")
                    print("negative 1k sampling for " +
                          G.name+" used: ", time.clock()-start)
